...
Talk to the user in their language. Run everything from the user's local machine with `ssh <host>`. `scripts/configure.sh` lives in this skill's directory (not the user's working directory) and is piped over stdin, so nothing is copied to the server.

How to call it, which matters:

- `ssh` joins its arguments into one string that the server's shell parses again, so put the whole remote command in double quotes and each argument in single quotes inside it: `ssh <host> "bash -s -- apply '<proxy-url>'" < <skill-dir>/scripts/configure.sh`.
- That is only safe when the URL has no `'`. Percent-encode every character of the proxy user and password other than letters, digits and `-._~` (`@` → `%40`, `$` → `%24`, `'` → `%27`…). A URL needs that anyway.
- The `<` redirect needs a POSIX shell (bash, Git Bash, WSL). Windows PowerShell has no `<`; use a bash shell there.
- Don't run `restart` from a Claude session that is itself running on that server: it stops the server hosting you. Tell the user to run it, or do it from their local machine.
...
- `http://` or `https://`, optionally with `user:pass@` (percent-encoded, see above). Reject `socks5://` and `socks5h://`: Claude Code does not support SOCKS. A Clash/mihomo mixed port also speaks HTTP, so suggest `http://` on the same port.
...
```bash
ssh <host> "bash -s -- check '<proxy-url>'" < <skill-dir>/scripts/configure.sh
```

It prints the HTTP status of `api.anthropic.com` direct and through the proxy.

- Direct `403` (or `000`, a timeout): the server can't reach Anthropic on its own; the proxy is needed.
- Direct `401` `404` `405`: the server already reaches Anthropic. A proxy is only worth it if the user wants a specific egress. If they came here because a session hangs, say the network is probably not the cause and don't push the proxy.
...
```bash
ssh <host> "bash -s -- apply '<proxy-url>' '<no-proxy>'" < <skill-dir>/scripts/configure.sh
```
...
```bash
ssh <host> "bash -s -- restart" < <skill-dir>/scripts/configure.sh
```
...
```bash
ssh <host> "bash -s -- verify" < <skill-dir>/scripts/configure.sh
```
...
The script finds processes by the app's current layout (`~/.claude/remote/srv/…/server`, `~/.claude/remote/ccd-cli/…`). If it reports nothing while a session is clearly open, the layout has changed: look with `ps -u $(id -u) -o pid,args | grep claude` and adjust.
...
- Commands Claude runs in the remote session (`curl`, `git`, `npm`, `pip`…) inherit the proxy too. With a reverse tunnel, that traffic goes through the user's own connection.
